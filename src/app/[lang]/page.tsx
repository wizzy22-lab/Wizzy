...
export default async function Home({ params }: PageProps<"/[lang]">) {
  ...
  return (
    <main className="relative min-h-screen w-full bg-bg font-sans">
      {/*
        No wrapper around the page any more. The intro used to be a card blown
        up to cover the screen, which meant every element it hid had to sit
        under the one carrying its phase; the ring's intro is the ring's own
        arrival, so it needs nothing above itself and the whole page stays
        server-rendered.
      */}
      ...
      <div
        data-theme="dark"
        // `overflow-hidden` is what clips the ring. `RADIUS_K` puts its outer
        // cards past both edges of the window on purpose, and this is where
        // that stops being a horizontal scrollbar. Nothing lifts it now — the
        // intro no longer grows anything beyond this band.
